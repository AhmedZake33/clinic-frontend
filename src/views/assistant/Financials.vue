<template>
  <div>
    ...
    <div v-else>
      ...
      <b-card
        v-if="summary.by_doctor && summary.by_doctor.length > 1"
        class="mb-2"
      >
        <div
          class="d-flex align-items-center justify-content-between mb-1"
          style="cursor:pointer"
          @click="showByDoctor = !showByDoctor"
        >
          <h5 class="mb-0">
            {{ $t('financial.byDoctorTitle') }}
          </h5>
          <feather-icon :icon="showByDoctor ? 'ChevronUpIcon' : 'ChevronDownIcon'" />
        </div>
        <div v-if="showByDoctor">
          <b-row
            v-for="doc in summary.by_doctor"
            :key="doc.doctor_id"
            class="mb-1 align-items-center border-bottom pb-1"
          >
            <b-col
              cols="12"
              md="3"
            >
              ...
              <b-badge
                :variant="doc.doctor_role === 'sub-doctor' ? 'light-info' : 'light-primary'"
            ...
            >
              <b-badge
                ...
                class="mr-50"
              >
                {{ doc.paid_count }} {{ $t('financial.paid') }}
              </b-badge>
              <b-badge
                variant="warning"
                class="mr-50"
              >
                {{ doc.partial_count }} {{ $t('financial.partial') }}
              </b-badge>
              <b-badge variant="danger">
                {{ doc.unpaid_count }} {{ $t('financial.unpaid') }}
              </b-badge>
            </b-col>
          </b-row>
        </div>
          ...
          <b-row>
            ...
            >
              <b-form-input
                ...
                :placeholder="$t('financial.searchByClient')"
              />
            </b-col>
            <b-col
              cols="6"
        ...
        >
          ...
          <template #cell(actions)="data">
            <b-button
              ...
              size="sm"
              class="mr-1"
              @click="viewFinancial(data.item)"
            >
              <feather-icon icon="EyeIcon" />
            </b-button>
            <b-button
              v-if="parseFloat(data.item.remaining) > 0"
              v-b-tooltip.hover
              :title="$t('financial.pay')"
              variant="success"
              size="sm"
              class="mr-1"
              @click="viewFinancial(data.item); $nextTick(() => { activeTab = 1 })"
            >
            ...
            <b-button
              ...
              variant="danger"
              size="sm"
              @click="deleteFinancial(data.item)"
            >
        ...
        <div v-if="selectedFinancial">
          ...
          >
            ...
            >
              ...
              <div
                v-if="parseFloat(selectedFinancial.remaining) > 0"
                class="text-right mt-1"
              >
                <b-button
                  variant="success"
                ...
                >
                  <b-form @submit.prevent="addTransactions">
                    ...
                    >
                      ...
                      >
                        <b-form-group
                          :label="idx === 0 ? $t('transaction.amount') : ''"
                          label-size="sm"
                        >
                          <b-form-input
                            v-model="row.amount"
                      ...
                      >
                        <b-form-group
                          :label="idx === 0 ? $t('financial.paymentMethod') : ''"
                          label-size="sm"
                        >
                          <b-form-select
                            v-model="row.payment_method"
                            :options="txPaymentMethodOptions"
                            size="sm"
                            required
                          />
                        </b-form-group>
                      ...
                      >
                        <b-form-group
                          :label="idx === 0 ? $t('reservation.notes') : ''"
                          label-size="sm"
                        >
                          <b-form-input
                            v-model="row.notes"
                      ...
                      >
                        <b-button
                          v-if="txRows.length > 1"
                          variant="outline-danger"
                          size="sm"
                          @click="removeTxRow(idx)"
                        >
                    ...
                    <b-row>
                      ...
                      >
                        <small class="text-muted mr-1">
                          ...
                          <span
                            v-if="txRowsTotal > parseFloat(selectedFinancial.remaining)"
...
export default {
  ...
  methods: {
    ...
    resetFilters() {
      this.filters = {
        search: '', payment_status: '', payment_method: '', date_from: '', date_to: '',
      }
      this.pagination.current_page = 1
      this.fetchFinancials()
      this.fetchSummary()
    ...
    showAddModal() {
      ...
      this.editId = null
      this.form = {
        reservation_id: null, amount: '', paid: '', payment_method: 'cash', notes: '',
      }
      this.modalShow = true
    },
    showEditModal(item) {
