<template>
  <div>
        ...
        <b-row>
          <b-col cols="12" md="6">
            <b-form-group :label="$t('printSettings.clinicName')">
              <b-form-input v-model="form.print_clinic_name" :placeholder="$t('clinic.name')" />
            </b-form-group>
          </b-col>
          <b-col cols="12" md="6">
            <b-form-group :label="$t('printSettings.clinicPhone')">
              <b-form-input v-model="form.print_clinic_phone" :placeholder="$t('client.phone')" />
            </b-form-group>
          </b-col>
          <b-col cols="12">
            <b-form-group :label="$t('printSettings.clinicAddress')">
              <b-form-input v-model="form.print_clinic_address" :placeholder="$t('client.address')" />
            </b-form-group>
          </b-col>
          <b-col cols="12">
            <b-form-group :label="$t('printSettings.headerText')">
              <b-form-textarea v-model="form.print_header_text" rows="3" />
            </b-form-group>
          </b-col>
          <b-col cols="12">
            <b-form-group :label="$t('printSettings.footerText')">
              <b-form-textarea v-model="form.print_footer_text" rows="3" />
            </b-form-group>
          </b-col>
          <b-col cols="12" md="4">
            <b-form-group :label="$t('printSettings.primaryColor')">
              <b-input-group>
                <b-form-input v-model="form.print_primary_color" type="color" class="print-color-input" />
                <b-form-input v-model="form.print_primary_color" maxlength="7" />
              </b-input-group>
            </b-form-group>
          </b-col>
          <b-col cols="12" md="4">
            <b-form-group :label="$t('printSettings.clinicNamePosition')">
              <b-form-select v-model="form.print_clinic_name_position" :options="clinicNamePositionOptions" />
            </b-form-group>
          </b-col>
          <b-col cols="12" md="4">
            <b-form-group :label="$t('printSettings.patientInfoPosition')">
              <b-form-select v-model="form.print_patient_info_position" :options="patientInfoPositionOptions" />
            </b-form-group>
          </b-col>
        </b-row>
    ...
    <b-card :title="$t('printSettings.preview')" class="mt-2">
      <div class="print-preview">
        <div class="print-preview-header" :class="clinicNamePositionClass" :style="{ backgroundColor: safeColor }">
          {{ form.print_clinic_name || $t('clinic.name') }}
        </div>
        <div :class="['mt-1', clinicNamePositionClass]">
          <div v-if="form.print_clinic_phone">{{ form.print_clinic_phone }}</div>
          <div v-if="form.print_clinic_address">{{ form.print_clinic_address }}</div>
          <div v-if="form.print_header_text" class="mt-50">{{ form.print_header_text }}</div>
        </div>
        <hr>
        <h5 class="text-center">{{ $t('reservation.printPrescription') }}</h5>
        <div v-if="form.print_patient_info_position === 'top'" class="preview-patient-info">
        ...
        <div v-if="form.print_patient_info_position === 'after_doctor'" class="preview-patient-info">
          <p><strong>{{ $t('client.name') }}:</strong> {{ $t('reservation.client') }}</p>
          <p><strong>{{ $t('client.phone') }}:</strong> 01000000000</p>
        </div>
        <p><strong>{{ $t('reservation.diagnosis') }}:</strong> ...</p>
        <p><strong>{{ $t('reservation.treatment') }}:</strong> ...</p>
        <div v-if="form.print_patient_info_position === 'bottom'" class="preview-patient-info">
          <p><strong>{{ $t('client.name') }}:</strong> {{ $t('reservation.client') }}</p>
          <p><strong>{{ $t('client.phone') }}:</strong> 01000000000</p>
        </div>
        <div v-if="form.print_footer_text" class="text-center small text-muted mt-2">
